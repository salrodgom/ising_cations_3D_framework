program main
 ...
 integer,parameter          :: n_configurations = 31
...
!
 do ii=1,n_configurations
  !write(6,'(i4.4)') ii
  write (file_name, '( "uniq_", i2.2, ".gin" )' )ii
   ...
   if(line(1:4)=='frac') then
    ...
    do i=1,n_atoms
     ...
     if(line(1:7)=='species') exit reading_file
      ...
      read(line( 1: 4),*) label(ii,i,1)
      read(line( 7:10),*) label(ii,i,2)
      read(line(12:20),*) cryst_coor(ii,i,1)
      read(line(22:30),*) cryst_coor(ii,i,2)
      read(line(32:40),*) cryst_coor(ii,i,3)
...
!!!
 ...
 if( err_apertura /=0 ) stop 'error gulp-3-subs/OUTSOD'
 read(line,*) k !<-N.configuraciones unicas
 read(132,'(a)',iostat=err_apertura) line
 if( err_apertura /=0 ) stop 'error gulp-3-subs/OUTSOD_modified'
