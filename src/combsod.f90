    PROGRAM combsod 
       ...
       INTEGER,PARAMETER :: NSPMAX=10 , NATMAX=20000, NOPMAX=10000, NCELLMAX=10000
       INTEGER,PARAMETER :: NLINEAMAX=400 
       ...
       CHARACTER,DIMENSION(NSPMAX) :: symbol*3, symbolmap*3 
...
! The components of the operators vector must be lower or equal to 0.5  (why??? added later)
!
       WRITE (*,*) "**************************************************************************** " 
       WRITE (*,*) "               SOD (Site Occupancy Disorder) program, version 0.26 " 
       WRITE (*,*) " " 
       WRITE (*,*) "              R. Grau-Crespo and S. Hamad, University College London " 
       WRITE (*,*) " " 
       WRITE (*,*) "                      Contact:  <r.grau-crespo@ucl.ac.uk> " 
       WRITE (*,*) "**************************************************************************** " 
       WRITE (*,*) " " 
       WRITE (*,*) " " 
...
!  For some reason the matrix multiplication didn't work in this machine and i had to expand it!
!                                 
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 ...
 255       continue
	...
	enddo
       ...
       do op1=1,nop1
        do i=1,3
          WRITE (*,222)(mgroup1(op1,i,j),j=1,3),vgroup1(op1,i)
  222  format(4(1x,f7.4))
        enddo
!	WRITE(*,*) op1
       enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Write a xyz file to visualize
! CAMBIAR Generalizar para grupos no ortorrombicos!!!

        WRITE(25,*) nat
        WRITE(25,*) 
        at=0
        do at=1,nat
                 WRITE(25,110) symbol(spat(at)),coords(at,1)*a,coords(at,2)*b,coords(at,3)*c
...
! For the first configuration (count=1)	
	...
	opc=1
     ...
     write(6,*) indcount,degen(indcount),equivconf(equivcount,1:nsubs),equivcount
                endif
        enddo
        WRITE (47,*) 0
 ...
 200               continue
	...
	do indcount=1,nic
	 WRITE(30,330) indcount,degen(indcount),indconf(indcount,1:nsubs)
         !write(6,*) indcount,degen(indcount),indconf(indcount,1:nsubs)
         !write(6,*) '#', equivconf(equivcount,1:nsubs)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 332   		format(a85)
		WRITE(indcount+10000,332) linea
	enddo
  99    continue
	rewind(41)

        WRITE(indcount+10000,211) a,b,c,alpha,beta,gamma
	WRITE(indcount+10000,*) "frac"
 211    format(6(f10.4,2x))
        do at=1,nat
		sp=spat(at)
		if (sp.ne.sptarget) then
                    WRITE(indcount+10000,331) symbol(sp),"core",coords(at,1),coords(at,2),coords(at,3)
	            if (ishell(sp).eq.1) WRITE(indcount+10000,331) symbol(sp),"shel",coords(at,1),coords(at,2),coords(at,3)
		else
		    att=at-atini+1
                    call member(nsubs,newconf,att,ifound)
		    if (ifound==1) then
		         WRITE (indcount+10000,331) newsymbol(1),"core",coords(at,1),coords(at,2),coords(at,3)
	                 if (newshell(1).eq.1) WRITE(indcount+10000,331) newsymbol(1),"shel",coords(at,1),coords(at,2),coords(at,3)
                    else
		         WRITE (indcount+10000,331) newsymbol(2),"core",coords(at,1),coords(at,2),coords(at,3)
	                 if (newshell(2).eq.1) WRITE(indcount+10000,331) newsymbol(2),"shel",coords(at,1),coords(at,2),coords(at,3)
		    endif
		endif
 331    format(a3,2x,a4,2x,3(f11.7,2x))
	...
	do l=1,NLINEAMAX
		...
		WRITE(indcount+10000,333) linea
 333		format(a85)
	enddo
 199    continue
	rewind(42)
        ...
        if (indcount<10) then
           ...
           WRITE (indcount+10000,*) "cutp 16.0 1.0"
           WRITE (indcount+10000,*) "switch rfo gnorm 0.001"
           WRITE (indcount+10000,*) "stepmx 0.01"
           WRITE (indcount+10000,334) "output xtl c0000",snumber
           WRITE (indcount+10000,334) "output arc c0000",snumber
        endif
        if ((indcount>9).AND.(indcount<100)) then
           ...
           WRITE (indcount+10000,*) "switch rfo gnorm 0.001"
           WRITE (indcount+10000,*) "stepmx 0.01"
           WRITE (indcount+10000,334) "output xtl  c000",snumber
           WRITE (indcount+10000,334) "output arc  c000",snumber
        endif
        if ((indcount>99).AND.(indcount<1000)) then
           ...
           WRITE (indcount+10000,*) "switch rfo gnorm 0.001"
           WRITE (indcount+10000,*) "stepmx 0.01"
           WRITE (indcount+10000,334) "output xtl   c00",snumber
           WRITE (indcount+10000,334) "output arc   c00",snumber
        endif
        if ((indcount>999).AND.(indcount<10000)) then
           ...
           WRITE (indcount+10000,*) "switch rfo gnorm 0.001"
           WRITE (indcount+10000,*) "stepmx 0.01"
           WRITE (indcount+10000,334) "output xtl    c0",snumber
           WRITE (indcount+10000,334) "output arc    c0",snumber
        endif
        if (indcount>9999) then
           WRITE (indcount+10000,*) "Error, too many configurations (>9999)! Calculation files not written!"
        endif
 334    format(a15,a4)

        CLOSE (indcount+10000)
        enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 299    continue
        ...
        WRITE(indcount+10000,212) "CELL ",a,b,c,alpha,beta,gamma
 212    format(a5,6(f10.4,2x))
        WRITE(indcount+10000,*) "SPACE FULL P1 1 1"
        WRITE(indcount+10000,*) "FRAC"
        do at=1,nat
                sp=spat(at)
                if (sp.ne.sptarget) then
...
!	title=symbol(1)//symbol(2)
        ...
        WRITE(indcount+10000,*) title
        WRITE(indcount+10000,*) '1.00000000' 
        call cell(cellvector, a, b,c,alpha,beta,gamma)
        WRITE(indcount+10000,335) cellvector(1,1), cellvector(2,1), cellvector(3,1) 
        WRITE(indcount+10000,335) cellvector(1,2), cellvector(2,2), cellvector(3,2) 
        WRITE(indcount+10000,335) cellvector(1,3), cellvector(2,3), cellvector(3,3) 
 335    format(3(f10.6,2x))
        ...
        WRITE(indcount+10000,336) snatsp(1:nsp+1) 
 336    format(10(i4,1x))
        WRITE(indcount+10000,*) 'Direct'
        do at=1,atini-1
           sp=spat(at)
           WRITE(indcount+10000,335) coords(at,1),coords(at,2),coords(at,3)
	enddo
        do at=atini,atfin
           sp=spat(at)
           att=at-atini+1
